function pathfinder_step(){ // Code that runs during a pathfinders step event
	with(other){
		...
		if (pathTimer = 0 && targetNode != undefined){ // When timer is over get new path
			...
			path = path_a_star(ghostNode, targetNode.id); // Pathfind from the ghostNode to the targetNode
		...
		if (arrived = false){
			...
			if (array_length(path) >= 2)if (collision_line(path[0][0].x + path[0][1], path[0][0].y + path[0][2], path[1][0].x + path[1][1], path[1][0].y + path[1][2], nbpCollision, true, true))pathTimer = irandom_range(0,4);
